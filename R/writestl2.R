...
#' @return None
#' @export
write_stl2<- function(df, filename){
	...
	pass_constructor<- function(...){
		...
		pass <- function(x,y){
			...
			invisible(Map(write,triangle_1,temp_file))
			invisible(Map(write,triangle_2,temp_file))
		}
		return(pass)
	}

	pass_1 <- pass_constructor(
				v_1=c(x,   y, 0),
				v_2=c(x+1, y, 0),
